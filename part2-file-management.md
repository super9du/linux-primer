...
> 摘自：https://www.cnblogs.com/lclq/p/5741852.html

##### `cat`

* `cat>>filename` 使用cat创建文件时，以系统默认的文件属性作为新文件的属性，并接受键盘输入作为文件的内容。输入结束时按`Ctrl+d`退出并保存文件。
* `cat filename` 查看文件内容
* `cat file1 file2>>file3`  将  file1 和 file2 合并到一个新的文件当中。（合并前需要确认这三个文件存在）

##### `touch`

主要用于修改指定文件的访问和修改时间属性

* 创建空文件夹：`touch ~/empty` 。如果该文件夹已存在将只更改文件夹的时间戳。
* 创建文件：`touch ~/hello.sh`

==注：根据多方查找，linux中似乎并不存在创建文件的同时创建目录的命令。因此在linux中创建文件需要先创建目录。==

### 文件链接

#### Linux 文件数据存储原理

> 摘自：https://www.ibm.com/developerworks/cn/linux/l-cn-hardandsymb-links/index.html
...
> 详见：https://www.cnblogs.com/li-hao/archive/2011/10/03/2198480.html
